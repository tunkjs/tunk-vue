                ...
                if (this.$options.state) {
                    ...
                    for (var x in stateOptions) if (stateOptions.hasOwnProperty(x)) {
                        if (typeof stateOptions[x] === 'string' && stateOptions[x].indexOf('.') > -1) {
                            ...
                            //this.$options.data = function(){ return data();};
                            //this.$options.data[x] = tunk.connection.state(this, x, stateOptions[x].split('.'));
                            // this.$set(x, tunk.connection.state(this, x, stateOptions[x].split('.')));
                            //console.log(this);
                            //this[x]=tunk.connection.state(this, x, stateOptions[x].split('.'));
